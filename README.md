...
| **一键配置环境** | 一次点击体检 9 项（指针 / 网关源码 / 解释器与依赖 / 端口 / API Key / 启动参数 / 脱敏补丁 / 内置 WebUI / 旧计划任务），能修的当场修好并复检，全程逐行回显；缺依赖会按「源 × 代理/直连」阶梯联网装包，**优先建独立虚拟环境，不污染系统 Python** |
| **环境自愈** | 每次启动自动修正失效的网关目录、Python 路径、被占用的端口 |
| **首次向导** | 4 步完成全部配置，含解释器自动探测；运行环境页带「一键修复环境」（等价于设置页那一个，但不用先进主界面） |
...
| 一键配置环境**不**自动装 Python 本体、不删旧计划任务、不启动网关 | 装 Python 要 UAC，而且装错位数（32 位包）比没装更难查；删计划任务是会影响系统的动作，得由用户在明确按钮上确认；启动网关是用户的决定。这三件事给入口、给说明，但不替他做 |
