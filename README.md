...
| iOS 开关**继承** `QCheckBox` 自绘，而不是造新控件 | 全项目 16 处 `QCheckBox(...)` 创建点 + 一堆 `isChecked/setChecked/toggled` 调用点（含 215 项布局自测）。继承之后这些调用点一个字都不用改，`isinstance(w, QCheckBox)` 也继续成立；而 QSS 的 `::indicator` 只能画矩形/圆角块，做不出「圆钮在滑轨上平移」这件事 |
