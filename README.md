...
# [6d] 段钉住「网关目录必须带 app/ 包」：只有 converter.py 的目录不算合格，
# 体检要判 fix、启动前要拦下（不白起子进程）、修复要就地补齐而不是换目录
python tests/selftest.py

# GUI 布局 / 交互回归自测（244 项，无头可跑）
...
| 网关目录找不到时，**去问正在运行的网关进程**（命令行里有 `…/converter.py serve`），不要只靠 `app_root()` 逐级猜 | 网关源码放哪儿是用户的自由。猜不中的表现是向导把一个**不存在的路径**填进输入框，用户只看到「网关目录里找不到 converter.py，请确认目录是否正确」—— 可机器上明明有人知道答案。命令行的路径要**按 token 取**（只认以 `converter.py` 结尾的那个参数），用正则从第一个盘符开始扫会把 `pythonw.exe` 和网关路径一起吞进去，拼出的垃圾路径再被 `is_gateway_dir()` 否掉 → 功能静默失效 |
| 采纳"运行中的那个网关"时，**端口也要一起采纳** | 只改目录不改端口的话，照向导点完会在**同一份源码目录**上再起一个实例 —— 两个进程同时写同一份 `auth/` 与 `.env`，比端口冲突更难查 |
...
| 「是不是网关目录」不能只看 `converter.py` 在不在，还要看它 `import` 的东西在不在 | 上游 `converter.py` 第 1 段就是 `from app.adapters.responses_adapter import …`，而 converter.py 在不在**完全说明不了** `app/` 在不在：半途中断的下载、手工拷了一半的目录，都会留下「有 converter.py、没有 app/」。这种目录以前被判成合格，一路通过体检 / 向导 / 启动，子进程只在第 40 行抛一句 `ModuleNotFoundError: No module named 'app'` 并以退出码 1 死掉（2026-09-23 用户贴来的 `gateway.log` 里 8 段 traceback 全是这一句）。判据改成「两个都在」，且第二个是**从 converter.py 自己的源码里读出来**的（`_gateway_needs_app()`，带 `(size, mtime_ns)` 缓存）—— 不写死 `app/`，上游哪天换布局也不会变成永远修不好的假警报 |
| 不完整的网关目录要**就地补齐**，不能换目录、更不能等它自己好 | 三种坏法三种修法：① 目录在、就是缺东西（典型：缺 `app/`）→ 就地补齐。`apply_release()` 按顶层条目逐个覆盖且跳过 `KEEP_PATHS`，所以 `auth/`、`.env`、启动脚本都留得住 —— 用户路径不变、凭证不丢；② 别处有一份能用的 → 改用它，不下载；③ 哪儿都没有 → 从上游拉一份。① 的入口条件是 **`converter.py` 在 + 整体不合格**两个都成立：只看 `is_dir()` 的话，用户把空目录甚至整个盘符填进配置时，我们会把一份网关源码解到那儿去。另外 `ensure_ready()` 以前用 `(gw/converter.py).is_file()` 判断，于是坏目录会被写进 `app.last_gateway_dir`，`locate_gateway_dir()` 下次还把它选回来 —— 坏目录永远自愈不了 |
| 必然失败的启动要在**起子进程之前**拦下 | 「退出码 1 + traceback」虽然比原来那句「进程提前退出（退出码 1）」信息量大得多，但它终究是让用户先等十几秒、再读一段栈。缺 `app/` 是**可以提前判定**的，所以 `GatewayManager.start()` 第一件事就是验目录，直接给「缺什么 + 点哪个按钮补」，连子进程都不起。这条特意**不收宽**：判据来自 converter.py 自己的 import，自定义布局的网关不会因为"没有 app/ 目录"被误拦 |
...
**Q：日志里刷 `ModuleNotFoundError: No module named 'app'`（退出码 1）？**
说明网关源码目录**不完整**：`converter.py` 在，但它第 1 段就要
`from app.adapters.responses_adapter import …`，而 `app/` 包不在。
半途中断的下载、手工拷了一半的目录都会这样。
到「设置 → 运行环境」点「一键配置环境」（向导里的「一键修复环境」同理）：
它会**就地补齐** —— 路径不变，`auth/` 登录凭证与 `.env` 都会保留，
不会另拉一份源码把你现有的东西甩掉。
（1.1.6 起，这种目录在体检里就会被判成「网关目录不完整（缺 app/__init__.py）」，
点启动也会在起进程**之前**直接告诉你怎么补，不再先等十几秒再丢一段 traceback。）
